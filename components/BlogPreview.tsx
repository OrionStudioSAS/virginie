...
export const BlogPreview: React.FC = () => {
  ...
  const cards = combined.map((item) => (
    <Link
      ...
      className="group bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-all duration-300 hover:-translate-y-1 flex flex-col h-full"
    >
      {item.image && (
