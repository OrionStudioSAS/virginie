      ...
      <div className="container mx-auto px-6 flex flex-col md:flex-row items-center gap-12 relative z-10">
        <div className="flex-1 space-y-8 text-center md:text-left">
          <div className="reveal reveal-slide-left">
            <h2 id="cms-hero_badge" className="inline-block bg-rose-50/80 text-primary font-semibold uppercase tracking-widest text-xs md:text-sm mb-3 px-4 py-1.5 rounded-full shadow-sm">
              Diététicienne Nutritionniste
            </h2>
            <h1 id="cms-hero_titre" className="text-4xl md:text-6xl font-serif font-bold text-slate-800 leading-tight">
              Retrouvez votre équilibre avec un accompagnement <span className="text-[#8bc242] underline decoration-[#8bc242]/20 decoration-4 underline-offset-4">personnalisé</span>
            </h1>
          </div>

          <p id="cms-hero_description" className="reveal reveal-slide-left reveal-delay-100 text-lg text-slate-600 leading-relaxed max-w-lg mx-auto md:mx-0">
          ...
          <div className="reveal reveal-scale-up reveal-delay-200 flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <Button href={DOCTOLIB_URL} target="_blank" id="cms-hero_btn_rdv">
              Prendre rendez-vous
            </Button>
            <Button variant="outline" id="cms-hero_btn_savoir_plus" onClick={() => document.getElementById('about')?.scrollIntoView({ behavior: 'smooth' })}>
              En savoir plus
            </Button>
          </div>
        </div>
